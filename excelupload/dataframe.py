...
def data_crud(obj, val, user):
    ...
    for i in range(len(val)):
        if val[i] == 120:
            ...
            # if for checking the qty is in string format
            ...
            # for checking the unit price is in string format
            ...
            dfm.loc[count_dfm, 'sales_office'] = obj.loc[i, 'col_19']  # sales office
    ...
    if val:
        ...
        if not val:
            ...
            # create the list of the dataframe
            ...
            master_objs = [
                proformaItemMaster(
                    ...
                    SoldToAddress=row['sold_to_address'],
                    Shiptocode=row['ship_to_code'],
                    Shiptoaddress=row['ship_to_address'],
                    EndUserCode=row['end_user_code'],
                    EndUserAddress=row['end_user_address'],
                    Billtocode=row['bill_to_code'],
                    Billtoaddress=row['bill_to_address'],
                    CusRefDesc=row['cus_ref_desc'],
                    CusDeliverydate=row['cus_delivery_date'],
                    Incoterms=row['inco_terms'],
                    SalesOffice=row['sales_office'],
                    YilContactperson=row['yil_contact_person'],
