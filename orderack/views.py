...
from .serializer import orderAcknowledgementSerializer, orderAckSerializer, orderAcknowledgementHistorySerializer

from excelupload.models import proformaItemMaster, proformaItem
from excelupload.serializer import proformaItemMasterSerializer, proformaItemSerializer

from masterdata.models import projectManagerMaster
from masterdata.serializer import projectManagerMasterSerializer

from .export_to_excel import dataListToExcel

from .common import generate_pi_no

from masterdata.models import divisionMaster, regionMaster
from masterdata.serializer import divisionMasterSerializer, regionMasterSerializer
...
##                            data['order'][itemval]['MultiplyUnitPrice'] = float(data['order'][itemval]['MultiplyUnitPrice'] + data['order'][itemval]['PI_Freight'])
    ...
    # by gautam 02 feb 2024
    @action(methods=['get'], detail=False, url_path='print_sbi_bank/(?P<id>[0-9]+)')
    def generatePDFWithSBI(self, request, id):
        pi_instance = orderAcknowledgement.objects.get(pk=id)
        serializer = self.serializer_class(pi_instance, context={'request': request})
        data = serializer.data
        filename = "Invoice__%s.pdf" % pi_instance.OrderAckId
        ...
        proforma_item_instance = proformaItem.objects.filter(ProformaID=data['ProformaID'])
        proforma_item_serializer = proformaItemSerializer(proforma_item_instance, many=True,
        ...
        for index, j in enumerate(data['order']):
            for i in itemno:
                if i['ProformaItemid'] == j['ProformaItemid']:
                    ...
                    if j['TotalAmount'] != 0:
                        data['order'][itemval]['TotalAmount'] = float(
                            data['order'][itemval]['PI_Qty'] * data['order'][itemval]['UnitPrice'])
                        data['order'][itemval]['MultiplyUnitPrice'] = float(
                            data['order'][itemval]['PI_Qty'] * data['order'][itemval]['UnitPrice'])
        ...
        if data['ProjectManagerId'] != 0:
            projectmanager_instance = projectManagerMaster.objects.get(pk=data['ProjectManagerId'])
            projectmanager_serializer = projectManagerMasterSerializer(projectmanager_instance,
                                                                       context={'request': request})
            data['Project_Manager'] = projectmanager_serializer.data['EmployeeName']
        else:
            data['Project_Manager'] = ""
        ...
        pdf_response = PDFTemplateResponse(
            request=request,
            template='invoice_sbi_bank.html',
            filename=filename,
            context=data,
            cmd_options={
                'title': filename,
